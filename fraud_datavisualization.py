...
ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"{int(x):,}")) # Format the y-axis by adding commas for thousands, millions, billions etc. Easier to read larger numbers.
...
if "trans_date_trans_time" in df.columns:
    ...
    ax.set_xlabel("Hour")
    ax.set_ylabel("Transaction Count") # The y-axis represents the number of transactions that occurred in each hour, allowing us to see if there are specific times when fraud is more likely to happen.
...
#Plot shows how the transactions (fraud and non-fraud are distributed)
